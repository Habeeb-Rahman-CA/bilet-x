...
@Component({
  ...
  template: `
    <div
      *ngIf="isOpen"
      class="fixed inset-0 z-50 flex items-start justify-center bg-black/80 pt-20 backdrop-blur-xs select-none"
    ...
    >
      ...
      >
        ...
        <div class="relative flex items-center border-b border-neutral-800 pb-3">
          ...
          />
          <kbd
            class="border border-neutral-800 bg-neutral-950 px-1.5 py-0.5 font-mono text-[9px] text-neutral-400"
          >
            ESC
          </kbd>
        </div>
        ...
        <div class="mt-3 max-h-64 space-y-1 overflow-y-auto">
          <div
            *ngFor="let item of filteredItems(); let idx = index"
            ...
            class="flex cursor-pointer items-center justify-between border border-transparent px-3 py-2 text-xs transition hover:border-neutral-800 hover:bg-neutral-900"
            [ngClass]="{ 'border-neutral-700 bg-neutral-900': selectedIndex === idx }"
          >
            <div class="flex items-center space-x-3">
              <span class="font-mono text-[10px] tracking-wider text-neutral-400 uppercase">
                [{{ item.category }}]
              </span>
            ...
            <span
              *ngIf="item.shortcut"
              class="border border-neutral-800 bg-black px-1.5 py-0.5 font-mono text-[9px] text-neutral-400"
            >
              {{ item.shortcut }}
            </span>
          </div>
        ...
        <!-- Panel Footer -->
        <div
          class="mt-3 flex items-center justify-between border-t border-neutral-800 pt-2 text-[10px] text-neutral-500"
        >
